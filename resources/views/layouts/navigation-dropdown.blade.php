<header class="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
      <div class="container flex items-center justify-between h-full px-6 mx-auto text-purple-600 dark:text-purple-300">
        ...
        <div class="flex justify-center flex-1 lg:mr-32">
            <h2 class="text-black font-bold dark:text-gray-200">
                @if(Auth::user()->department_dept_id == 1)
                    {{ __('Elementary Department') }}
                @elseif(Auth::user()->department_dept_id == 2)
                    {{ __('Junior High School Department') }}
                @elseif(Auth::user()->department_dept_id == 3)
                    {{ __('Senior High School Department') }}
                @else
                {{ __('Super Admin') }}
                @endif
            </h2>
